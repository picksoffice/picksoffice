...
try {
  const darkPrefixes = execSync(
    'grep -r "dark:" --include="*.tsx" --include="*.ts" --include="*.css" ./src',
...
try {
  const bgWhite = execSync(
    'grep -r "bg-white" --include="*.tsx" --include="*.ts" --include="*.css" ./src',
    { encoding: 'utf8', stdio: ['pipe', 'pipe', 'ignore'] }
  );
...
try {
  const textGray900 = execSync(
    'grep -r "text-gray-900" --include="*.tsx" --include="*.ts" --include="*.css" ./src',
    { encoding: 'utf8', stdio: ['pipe', 'pipe', 'ignore'] }
  );
...
console.log(
  'Check complete. If any matches were found, you may need to manually update those files.'
);

    ...
    <div className="py-24 sm:py-32">
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="mt-10 flex items-center justify-center">
          <Button href="/picks" size="lg" className="rounded-full">
            View All Picks
          </Button>
        </div>
      </div>
      ...
      <article className="relative isolate flex flex-col justify-end overflow-hidden rounded-2xl bg-slate-800/30 backdrop-blur-sm shadow-lg border border-white/10 ring-1 ring-white/10 h-full transition-all group-hover:shadow-xl group-hover:bg-slate-800/40">
        ...
        <div className="flex flex-1 flex-col justify-end p-6">
          ...
          <div className="flex items-center gap-3 mb-3">
            ...
            <span className="text-xs text-gray-400">{formatDate(pick.Date)}</span>
          </div>
          ...
          <div className="inline-block bg-slate-800/60 rounded-lg px-3 py-2 border border-slate-700/50">
            <div className="flex items-center justify-between">
              <div>
                ...
                <span className="ml-2 text-gray-400">{formatAmericanOdds(pick.Odds)}</span>
              </div>
              {pick.Result !== 'Pending' && (
                <span
                  className={`ml-4 font-medium ${pick.Result === 'Win' ? 'text-emerald-400' : pick.Result === 'Loss' ? 'text-red-400' : 'text-gray-400'}`}
                >
          ...
          {pick.Summary && (
            <p className="mt-4 text-sm leading-6 text-gray-300 line-clamp-2">{pick.Summary}</p>
          )}
        </div>
      </article>
    </Link>

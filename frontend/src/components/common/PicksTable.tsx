...
export default function PicksTable({ picks, pagination }: PicksTableProps) {
  ...
  const loadMorePicks = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/picks?sort=Date:desc&populate=*&pagination[pageSize]=20&pagination[page]=${nextPage}`);
            ...
            <Table className="table-auto border-none min-w-full">
              ...
              <TableBody className="divide-y divide-gray-200 dark:divide-gray-700">
                {filteredPicks.map((pick) => (
                  ...
                  >
                    <TableCell className="text-gray-700 dark:text-gray-300">{pick.League}</TableCell>
                    <TableCell className="text-gray-700 dark:text-gray-300 whitespace-nowrap">{formatAmericanDate(pick.Date)}</TableCell>
                    ...
                    <TableCell className={`font-medium ${pick.Result === 'Win' ? 'text-emerald-400' : pick.Result === 'Loss' ? 'text-red-400' : 'text-gray-700 dark:text-gray-300'}`}>

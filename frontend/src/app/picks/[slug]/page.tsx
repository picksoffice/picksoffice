...
export async function generateMetadata({ params }: Props) {
  try {
    try {
      const response = await fetchAPI<Pick>('/picks', {}, `filters[Slug][$eq]=${encodeURIComponent(params.slug)}&publicationState=live`);
      console.log('Metadata API Response for slug:', params.slug, JSON.stringify(response, null, 2));
...
export default async function PickDetailPage({ params }: Props) {
  ...
  try {
    ...
    try {
      const response = await fetchAPI<Pick>('/picks', {}, `filters[Slug][$eq]=${encodeURIComponent(slug)}&publicationState=live`);
